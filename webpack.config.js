...
const configEnvVals = {};
configEnvVals.filename = 'publishing-ui.min';
configEnvVals.entry = path.join(__dirname, '/app/scripts/bootstrap');
...
if (env === 'dev') {
    ...
    configEnvVals.filename = 'publishing-ui';
    configEnvVals.entry = path.join(__dirname, '/app/scripts/testbed');
...
//base webpack config
let config = {
    entry: [configEnvVals.entry],
    output: {
        path: __dirname + '/lib',
        filename: `${configEnvVals.filename}.js`,
        umdNamedDefine: true
    },
    resolve: {
        root: [
